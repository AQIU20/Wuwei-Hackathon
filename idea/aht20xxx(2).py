...
class NodeMqttClient:
    ...
    def subscribe_sensor_data(self) -> None:
        """Add a dedicated subscription to sensor/data for this node (already covered by wildcard, kept for clarity)."""
        topic = self.build_topic("sensor", "data")
...
def command_status(client: NodeMqttClient, timeout: float, json_only: bool) -> int:
    ...
    response = client.wait_for_topic_suffix("/info", timeout)
    if response is None:
        print("timed out waiting for info response", file=sys.stderr)
        return 1

    envelope = extract_envelope_payload(response)
...
def command_ws2812(
    ...
    response = client.wait_for_topic_suffix("/ws2812", timeout)
    if response is None:
...
def command_pixel(client: NodeMqttClient, fill: Optional[str], pixels: Optional[str], timeout: float, json_only: bool) -> int:
    ...
    response = client.wait_for_topic_suffix("/pixel", timeout)
    if response is None:
...
def command_imu(client: NodeMqttClient, timeout: float, json_only: bool) -> int:
    """Fetch the latest IMU reading by requesting node status and extract the imu field."""
    topic = client.publish_info_request()
    if not json_only:
        print(f"published: {topic}")
    ...
    if not isinstance(imu, dict):
        print("imu field missing in device status — is this an IMU node?", file=sys.stderr)
        return 1
...
def command_hr(client: NodeMqttClient, timeout: float, json_only: bool) -> int:
    """Fetch the latest heart-rate / SpO2 reading by requesting node status and extract the hr field."""
    topic = client.publish_info_request()
    if not json_only:
        print(f"published: {topic}")
    ...
    if not isinstance(hr, dict):
        print("hr field missing in device status — is this a HR node?", file=sys.stderr)
        return 1
...
def command_sensor(client: NodeMqttClient, count: int, timeout: float, json_only: bool) -> int:
    """Wait for up to *count* live sensor/data messages from the node (works for all node types)."""
    if not json_only:
        print(f"waiting for {count} sensor/data message(s) from {client.node_id} (timeout {timeout:.1f}s each)...")
