...
export function ContextMachine() {
  ...
  async function fetchGallery() {
    try {
      const res = await fetch(`${getAgentServerUrl()}/v1/gallery?limit=50`);
      if (res.ok) {
        const data = await res.json();
        setGallery(data.items || []);
      }
    } catch {
      // gallery unavailable, silent
    }
  }
  ...
  async function postToGallery() {
    if (!cardImage) return;
    try {
      await fetch(`${getAgentServerUrl()}/v1/gallery`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
    ...
    <div className="relative mx-auto max-w-7xl px-4 pt-10 pb-20 sm:px-6 lg:px-10">
      ...
      <div className="grid gap-6 lg:grid-cols-[220px_1fr]">
                ...
                <div
                  ...
                  className={`flex items-center gap-2 rounded-xl border px-3 py-2 text-sm transition-all ${
                    isActive
                      ...
                      : "border-black/10 bg-white cursor-grab hover:border-black/20 hover:shadow-sm active:cursor-grabbing"
                  }`}
                >
                  <span className="text-lg">{s.icon}</span>
                  <span className="font-medium">{sensorName(s.id)}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="space-y-6">
                ...
                {active.map((id) => {
                  ...
                  return (
                    <motion.button
                      ...
                      className="flex items-center gap-1.5 rounded-full bg-white/90 backdrop-blur-sm border border-black/10 px-3 py-1.5 text-xs font-medium shadow-sm hover:bg-red-50 hover:border-red-200 transition-colors"
                    >
                      <span>{s.icon}</span>
                      {sensorName(id)}
              ...
              <AnimatePresence>
                {unlocked.map((key) => (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: -10 }}
                    className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-violet-500 to-fuchsia-500 px-4 py-2 text-sm font-semibold text-white shadow-lg"
                  >
                    <span>✨</span>
          ...
          {/* Card preview (hidden, used for image generation) */}
          ...
          <AnimatePresence>
            {cardImage && (
              ...
              >
                ...
                {/* Actions row */}
                <div className="mx-auto flex max-w-md flex-col gap-4 sm:flex-row sm:items-end">
                  {/* Download */}
                  <a
                    href={cardImage}
  ...
  return (
    <div className="text-white">
        ...
        <div className="mt-2 flex flex-wrap gap-1.5">
          {active.map((id) => {
            ...
            return (
              <span
                key={id}
                className="inline-flex items-center gap-1 rounded-full bg-white/10 px-2.5 py-1 text-xs"
              >
      ...
      <div className="mt-6 flex items-center justify-between border-t border-white/10 pt-4">
        <span className="text-[10px] text-white/30">unforce-make.vercel.app</span>
        <span className="text-xs font-medium text-white/50">无为创造</span>
      </div>
    </div>
  );
}
